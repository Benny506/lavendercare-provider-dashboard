...
const DoctorSchedule = () => {
  ...
  const handleNavigate = (action) => {
    ...
    else if (action === 'today') return setCurrentDate(new Date());
    setCurrentDate(newDate);
  };

  const getMonthYearString = () => format(currentDate, 'MMMM yyyy');
  ...
  const eventPropGetter = (event) => {
    ...
    return { className: `!rounded !px-2 !py-1 !text-sm !font-medium !border-l-4 ${colorClass}` };
  };
  ...
  return (
    <div>
      ...
      <div className="min-h-screen">
        <div className="flex flex-col md:flex-row w-full max-w-7xl mx-auto gap-4 md:gap-0">
          {/* Sidebar */}
          ...
          {/* Calendar Content */}
          <div className="w-full md:flex-1 bg-white rounded-lg md:rounded-l-none md:rounded-r-lg p-2 md:p-6 shadow-sm border border-gray-200">
            {/* Header */}
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between mb-8 gap-4 lg:gap-0">
              ...
              <div className="flex bg-gray-100 rounded-lg p-1">
                {['day', 'week', 'month'].map((v) => (
                  <button
                    key={v}
                    className={`px-4 py-2 rounded-md text-sm font-medium transition-colors capitalize ${view === v
                        ? v === 'month'
            ...
            {/* Calendar Container with no horizontal overflow */}
            <div className="overflow-x-auto">
              <div className="min-w-full h-[600px]">
                <Calendar
                  localizer={localizer}
                  events={events}
                  startAccessor="start"
                  ...
                  toolbar={false}
                  className="custom-calendar bg-white min-w-full"
                />
              </div>
            </div>
          </div>
